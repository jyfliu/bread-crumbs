...
class Game:
  ...
  def __init__(self, config):
    self.config = config
    self.players = {}
    self.entities = [] # non player entities
    # TODO entities should be a set so its easy to remove them
    self.keys = set()
  ...
  async def tick(self, delta):
    ...
    await se.sio.emit('update',
      [(p.x, p.y, p.sprite_id) for p in self.players.values()] +
      [(e.x, e.y, e.sprite_id) for e in self.entities],
    )
